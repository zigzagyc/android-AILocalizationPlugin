...
class GrokService : TranslationService {
    ...
    private fun getModelCandidates(): List<String> {
        return listOf(model, "grok-beta", "grok-2", "grok-2-mini", "grok-vision-beta").distinct()
    }
    ...
    override suspend fun translate(text: String, targetLang: String, context: String?, apiKey: String): String = withContext(Dispatchers.IO) {
        ...
        for (candidateModel in getModelCandidates()) {
            ...
            if (response.statusCode() == 200) {
                ...
                if (response.statusCode() == 404 || errorBody.contains("not found", ignoreCase = true) || errorBody.contains("does not exist", ignoreCase = true)) {
                    lastErrorMsg = errorBody
                    continue
                }
                throw RuntimeException("Grok translation failed: $errorBody")
            }
        }

        throw RuntimeException("Grok translation failed: $lastErrorMsg")
    }
